...
func (userControler *UserController) LoginUser(context *gin.Context) {
	...
	// all ok
	// create general response
	response := Response.GeneralResponse{Error: false, Message: "your login is successful", Data: User2.LoginUserResponse{UserName: userResponse.UserName, AccessToken: userResponse.AccessToken, RefreshToken: userResponse.RefreshToken}}
	context.JSON(http.StatusOK, gin.H{"response": response})
}
